...
function setUpInitialThings() {
    ...
    echo "replica-announce-ip $HOSTNAME.$VALKEY_GOVERNING_SERVICE" >>/data/default.conf
}
...
function waitToSyncSentinelConfig() {
    ...
    for ((j = 0; j < $sentinel_replica_count; j++)); do
        ...
        if [[ "${#REPLICAS_INFO_FROM_SENTINEL}" == "0" ]]; then
            ...
            findSDownStateForAllReplicasInfo "$HOSTNAME.$VALKEY_GOVERNING_SERVICE" "$REPLICAS_INFO_FROM_SENTINEL"
            if [ "$s_down" == "true" ]; then
                resetSentinel "$SENTINEL_NAME-$j"
            fi
        fi
...
not_exists_dns_entry() {
    ...
    log "INFO" "Check if $VALKEY_GOVERNING_SERVICE contains the IP of this pod: ${myip}"
    if [[ -z "$(getent ahosts "$VALKEY_GOVERNING_SERVICE" | grep "^${myip}")" ]]; then
        flag=1
    else
        log "INFO" "$VALKEY_GOVERNING_SERVICE has my IP: ${myip}"
        flag=0
    fi
}
...
if [[ "${#VALKEY_SENTINEL_INFO[@]}" == "0" ]]; then
    ...
    self="$HOSTNAME.$VALKEY_GOVERNING_SERVICE"
    if [[ $HOSTNAME == "$VALKEY_NAME-0" ]]; then
        exec valkey-server /data/default.conf $args &
        pid=$!
        waitForValkeyToBeReady $self
        addConfigurationWithAllSentinel "$VALKEY_NAME-0.$VALKEY_GOVERNING_SERVICE"
    else
        while true; do
